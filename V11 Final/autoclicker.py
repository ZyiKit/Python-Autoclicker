...
def load_file(filename="settings.json"):
    ...
    try:
        ...
        #rint("load success") #debugging purposes
    except:
...
#FOR ENDING
def close():
    #print('sucess') for debugging purposes
    KB_listener.stop()
    MO_listener.stop()
    window.destroy() #be careful if you run window.destroy while python is running on the mouselistener or keyboard listener thread it will not run as tkinter designed this function to be run in the main thread only
    sys.exit()
...
def on_press(key):
    global activated, keybind_select, sleeping, keybind, setsleeping, Killswitch, Mode
    if keybind_select == 'None':
        if Mode.get() == 0 and not activated: #0 means hold
            try:
        ...
        elif Mode.get() == 1: # 1 means toggle mode
            try:
                if (key.char).lower() == (keybind.char).lower():
                    sleeping = setsleeping
        ...
        try:
            if (key.char).lower() == (Killswitch.char).lower():
                window.after(0, close) #ensures that the close() function is called on tkinter main loop to ensure tkinter window thread closes properly

        except AttributeError:
            if key == Killswitch:
                window.after(0, close)
    elif keybind_select == 'Keybind':
        if key != Killswitch:
            keybind = key
            keybind_label.config(text='Activate Keybind: {}'.format(keybind))
            time.sleep(0.1)
            ...
            keybind_label.config(text='Keybind cannot be same as Killswitch')
            time.sleep(0.1)
            keybind_select = 'None'
        ...
        if key != keybind:
            ...
            keybind_label_KS.config(text='Killswitch cannot be same as Keybind')
            time.sleep(0.1)
            keybind_select = 'None' # keybind_select is a variable that tells python another listener for reset keybind hasn't been opened 


        

def on_release(key):
    global activated, keybind, keybind_select
    if Mode.get() == 0 and keybind_select == 'None':
        try:
            if (key.char).lower() == (keybind.char).lower():
                activated = False
        except AttributeError:
...
def on_click(x,y,button, pressed):
    global activated, keybind, sleeping, setsleeping, keybind_select
    if button == keybind and keybind_select == 'None':
        if Mode_list[Mode.get()] == 'Toggle' and pressed: #need and pressed so it only flicks the activation switch onces, otherwise each click has press and release which counts as two which just turns it on and off
            activated = not activated
            if not activated:
                sleeping = setsleeping
        ...
        if pressed and button not in invalid_list:
            if keybind_select == 'Keybind':
                ...
                keybind_label_KS.config(text='Activate Killswitch Keybind: Mouse button cannot be accepted')
                time.sleep(0.1)
                keybind_select = 'None'
...
if __name__== "__main__":
    ...
    Mode = IntVar()
    Mode.set(0) # 0 is hold, 1 is toggle
    Mode_list = ["Hold", "Toggle"]
    Hold_Title = Label(window, text=f"Mode: {Mode_list[Mode.get()]}")
    ...
    ClickButton.set(0) #0 left, 1 right, 2 middle
    ClickButton_Title = Label(window, text=f"Click Button: {ClickButton_list[ClickButton.get()]}")
    ...
    KB_listener = KeyboardListener(on_press=on_press, on_release=on_release, daemon=True)
    MO_listener = MouseListener(on_click=on_click, daemon=True)
