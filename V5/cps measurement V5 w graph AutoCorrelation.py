...
def plot_graph():
    ...
    plt.plot(x2,y2, c="red", linewidth=2, label="Instantaneous cps")
    plt.plot(x1,y1, c="green", linewidth=2, label="Average cps")
    plt.ylim(0,(max(y2)+5))
    plt.xlabel("Time")
    plt.ylabel("CPS")
    plt.title("CPS TEST")
...
def autocorrelation():
    ...
    intervals=np.diff(goodlist)
    autocorrelationy = []
    kx = []
    totalsum: int = 0
    for x in intervals:
        totalsum+=x
    mean = totalsum/(len(intervals))
    difflist = []
    for x in range(0, len(intervals)):
        difflist.append(intervals[x]-mean)
    ...
    for k in range(1, 15):
        kx.append(k)
        top = 0
        for index in range(0, len(intervals)-k):
            top += (difflist[index])*(difflist[index+k])
            
        autocorrelationy.append(top/bottom)

    plt.plot(kx, autocorrelationy, c="blue", linewidth=2)
    plt.xlabel("Lag")
