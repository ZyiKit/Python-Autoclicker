...
from pynput.mouse import Listener as MouseListener, Button as MouseButton, Controller #PYthon stores buttons as MouseButton.left, MouseButton.x2 internally it is just they still print Button.x2 or Button.left due to internal stuff
from pynput import keyboard                                                           #Same goes for keyboard
import time
import threading
import random
import numpy as np
...
#OTHER VARIABLES , (SETTINGS HERE ARE THE DEFAULT SETTINGS)
keybind = None
Killswitch = None
...
def load_file(filename="settings.json"):
    ...
    try:
        ...
        keybind_label_KS.config(text='Activate Kill Switch Keybind: {}'.format(Killswitch))
...
def on_press_hold(key):
    global activated, sleeping, keybind, setsleeping, Killswitch
    print(f"keybind repr {repr(keybind)}")
    print(f"type keybind {type(keybind)}")
    ...
    if key == Killswitch:         #Users should be able to pick kill switch
        ...
        return
    try:
        if key == keybind and (not activated):
            print("activated") #Debugging purposes
            activated = True
            sleeping = setsleeping
    except AttributeError:
        if key == keybind and not activated: #for keys like esc, shift and ctrl
            print("activated") #Debugging purposes
            activated = True 
            sleeping = setsleeping
        pass
...
def on_release_hold(key):
    ...
    try:
        if key == keybind: #key.char is string
            print("Deactivated") #Debugging purposes
            activated = False
    except AttributeError:
        if key == keybind: #for keys like esc, shift and ctrl
            print("Deactivated") #Debugging purposes
            activated = False
        pass
...
def perform_macro():
    ...
    while True:
        if activated:
            mouse_controller.click(MouseButton.left, 1)  #Button.left should be a variable that users can pick
            while True:
...
window.protocol("WM_DELETE_WINDOW", close) #OPENS the close function to close the system properly when user clicks x, WM_DELETE_WINDOW is python's way of knowing when window closes it

if Mode_list[Mode.get()] == 'Toggle':
    KB_listener = KeyboardListener(on_press=on_press_toggle, daemon=True)
    MO_listener = MouseListener(on_click=on_press_mousecon, daemon=True)
    ...
    KB_listener = KeyboardListener(on_press=on_press_hold, on_release=on_release_hold, daemon=True)
    MO_listener = MouseListener(on_click=on_press_mousecon, daemon=True)
